  ...
  return (
    <div className="flex flex-wrap gap-2">
      {Object.entries(cost).map(([key, amount]) => {
        ...
        return (
          <span className="rounded-full bg-slate-100 px-2 py-1 text-xs font-bold text-slate-700" key={key}>
            {amount} {meta.label}
          </span>
        );
      })}
  ...
  return (
    <div className="flex flex-wrap gap-2">
      ...
      {Object.entries(building.passive).map(([key, amount]) => {
        const meta = resourceMeta[key as ResourceKey];
        return (
          <span className="rounded-full bg-teal-50 px-2 py-1 text-xs font-bold text-teal-800" key={key}>
    ...
    <article className="rounded-lg border border-slate-200 bg-white p-4">
      <div className="mb-3 flex items-start gap-3">
        ...
        <div className="min-w-0">
          <h3 className="truncate text-base font-bold">{building.name}</h3>
          <p className="truncate text-xs font-semibold text-slate-500">{building.lifeArea}</p>
        </div>
      </div>
      <p className="mb-3 min-h-12 text-sm text-slate-600">{building.description}</p>
      <CostLine cost={building.cost} />
      <div className="mt-3">
        <PassiveLine building={building} />
      </div>
      <button
        className="mt-4 flex h-10 w-full items-center justify-center gap-2 rounded-md bg-slate-900 text-sm font-bold text-white disabled:cursor-not-allowed disabled:bg-slate-200 disabled:text-slate-500"
