...
import { Check, Hammer, Home, Landmark, User } from "lucide-react";
...
function TopBar({ screen, onScreenChange }: { screen: Screen; onScreenChange: (s: Screen) => void }) {
  ...
  return (
    <header className="mb-4 flex flex-col gap-3 border-b border-slate-200 pb-4 md:flex-row md:items-center md:justify-between">
      <div>
        <p className="text-xs font-semibold uppercase tracking-wide text-teal-700">Future self builder</p>
        <h1 className="text-3xl font-bold leading-tight text-slate-950">Addy City</h1>
      </div>
      <div className="flex items-center gap-3">
        <nav className="grid grid-cols-5 rounded-lg border border-slate-200 bg-white p-1 shadow-sm">
          {tabs.map((tab) => {
            const Icon = tab.icon;
...
function SidePanel() {
  ...
  const stages = [
    ...
    { label: "City", active: population >= 500 && population < 5000, range: "500 – 5000" },
    { label: "Metropolis", active: population >= 5000, range: "5000+" },
  ] as const;
        ...
        <div className="space-y-3">
          {stages.map(({ label, active, range }) => (
            <div className="flex items-center justify-between gap-3" key={label}>
              <span className={`text-sm font-bold ${active ? "text-teal-800" : "text-slate-600"}`}>{label}</span>
          ...
          <div className="space-y-2">
            {rareDrops.slice(0, 4).map((drop) => (
              <div
                ...
                key={drop.id}
              >
                <span className="text-sm font-bold text-amber-900">{drop.name}</span>
                <span className="text-xs text-amber-700">{drop.chance}%</span>
              </div>
            ))}
          </div>
        ) : (
          <p className="text-sm text-slate-600">No rare monuments yet.</p>
        )}
      </Panel>
      <Panel title="City Status">
        <p className={`text-sm font-semibold ${cityPaused ? "text-amber-700" : "text-emerald-700"}`}>
          {cityPaused
            ? "Growth paused until you complete a task today."
            : `Growth active with a ${streak}-day streak.`}
        </p>
        <button
          className="mt-3 flex h-10 w-full items-center justify-center gap-2 rounded-md border border-slate-200 bg-white text-sm font-bold text-slate-700 hover:bg-slate-50"
          onClick={handleReset}
          type="button"
        >
...
function AppInner() {
  ...
  useEffect(() => {
    ...
    Promise.all([loadCity(), loadTasks()]).finally(() => setReady(true));
  }, []); // eslint-disable-line react-hooks/exhaustive-deps

  if (!ready) {
    return (
      <main className="flex min-h-screen items-center justify-center">
        <p className="text-sm font-semibold text-slate-500">Loading city…</p>
      </main>
    );
  }
